...
export default function Login() {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Welcome back to LifeSteal Shop!",
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-900 via-black to-cyan-900 flex items-center justify-center p-4">
      ...
      <Card className="w-full max-w-md bg-black/40 backdrop-blur-lg border-green-500/20 shadow-2xl">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription className="text-gray-300">
            Sign in to your LifeSteal Shop account
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 shadow-lg hover:shadow-green-500/25"
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full bg-black/40 border-green-500/30 text-white hover:bg-green-600/20"
            onClick={() => {
              toast({
                title: "Google Login Temporarily Unavailable",
                description: "Please use email login for now. Google OAuth is being configured.",
                variant: "destructive"
              });
            }}
          >
